...
void PragmaLearnHandler::HandlePragma(Preprocessor &PP,
  ...
  PP.Diag(PragmaLoc, diag::warn_pragma_learn_activated) << Directive->getName();
  ...
  if (Directive->isStr("warn")) {
    ...
    if (Tok.is(tok::identifier)) {
      ...
      // If not identifier and not end of directive, warn but continue
      PP.Diag(Tok.getLocation(), diag::warn_pragma_extra_tokens_at_eol)
          << "learn";
    }

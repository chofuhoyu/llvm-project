...
但这个设计有一个问题：它不能区分不同类型的致命错误。

### 错误分类的必要性：为什么要拆分？

LLVM 15+将`report_fatal_error`拆分为两个函数，这不是随意的改动，而是基于实践经验的改进：
...
**设计权衡分析：**
- **为什么不都用abort()？**：用户错误不需要也不应该生成崩溃报告
- **为什么不都用exit()？**：内部错误需要详细的调试信息来诊断
- **为什么引入新函数而不是修改旧函数？**：保持API兼容性，渐进式迁移
...
从 [ErrorHandling.h](/llvm/include/llvm/Support/ErrorHandling.h#L36) 的注释可以看到这种谨慎：
...
**为什么如此谨慎？**
- **异常安全**：LLVM代码不是为异常安全设计的
- **资源泄漏**：异常可能导致资源无法正确释放
- **不变量破坏**：异常可能在破坏不变量的位置抛出
- **调试困难**：异常会使问题定位更复杂
...
**为什么不用assert(false)？**
- **语义更明确**：llvm_unreachable专门标记不可达代码
- **优化器理解**：优化器特别处理llvm_unreachable
- **更好的消息**：自动包含文件名和行号
...
这是一个非常关键的设计点。为什么需要这个标志？

1. **错误不能被忽略**
   - 如果创建了一个Error但没有检查它，析构时会触发运行时错误
   - 这强制程序员正确处理错误

2. **检查方式**
   - 成功状态：转换为bool即可标记为已检查
   - 失败状态：必须使用handleErrors或handleAllErrors处理
...
`Expected<T>`（或早期的`ErrorOr<T>`）包装可能失败的操作：
...
LLVM选择Expected<T>是一个平衡的选择：
- 避免了异常的性能和安全问题
- 比错误码+输出参数更优雅
- 强制错误检查（通过Error的Checked标志）
...
**为什么这样设计？**
- **ID用于类型鉴别**：不需要完整的虚函数表，只需要一个指针
- **log()用于显示**：统一的错误显示接口
- **convertToErrorCode()用于兼容**：与传统std::error_code互操作

### 错误处理函数：模式匹配风格

LLVM提供了`handleErrors`和`handleAllErrors`，实现了类似模式匹配的错误处理：
...
2. **未处理错误的处理**
   - handleErrors返回未处理的错误
   - handleAllErrors要求所有错误都被处理
   - 灵活性与安全性的平衡
...
**为什么不直接用switch-case？**
- **类型安全**：switch-case无法处理类型分派
- **扩展性**：添加新错误类型不需要修改现有代码
- **表达式风格**：可以作为表达式使用，更灵活
...
3. **性能考虑**
   - raw_ostream比std::ostream更高效
   - 更少的虚函数调用
   - 更好的缓冲策略

### LLVM_DEBUG：条件编译的艺术

[LLVM_DEBUG](/llvm/include/llvm/Support/Debug.h) 宏是条件调试输出的经典设计：

**设计要点分析：**

1. **编译期条件**
   - 调试代码在Release模式下完全不编译
   - 零运行时开销
   - 不会影响发布版本性能

2. **按类型启用**
   - DEBUG_TYPE宏定义当前模块的调试类型
   - 运行时可以选择性启用某些类型的调试输出
   - 灵活控制调试信息量
...
**与NDEBUG的关系：**
- NDEBUG控制assert和LLVM_DEBUG
- Debug模式：NDEBUG未定义，启用所有检查和调试
- Release模式：NDEBUG定义，禁用检查和调试
...
从 [llvm/CMakeLists.txt](/llvm/CMakeLists.txt) 可以看到，断言受CMake选项控制：
...
- **地址信息不友好**：原始堆栈跟踪只有地址
- **缺少上下文**：不知道每个函数在做什么
- **Release模式问题**：没有调试符号时更糟

**PrettyStackTrace的解决方案：**
- **人工维护的堆栈**：显式跟踪当前操作
- **有意义的描述**：描述正在做什么，而不是在哪里
- **独立于调试符号**：即使在Release模式也有用
...
LLVM的错误处理和调试机制体现了以下核心设计思想：

1. **多层防御**：从assert到Error，多层次的错误处理
2. **零开销原则**：Release模式不支付未使用功能的成本
3. **安全与性能平衡**：Debug模式安全，Release模式快速
4. **诊断友好**：PrettyStackTrace等机制辅助调试
5. **强制错误检查**：Error的Checked标志防止忽略错误

理解这些设计理念，不仅能帮助我们正确使用LLVM，更能学习到工业级C++项目的错误处理最佳实践。
